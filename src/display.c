/*
 * Leveraging code from ECEN-5823 IoT Embedded Firmware Class
 * for the display
 *
 * display.c
 *
...
#include "timers.h" // Add a reference to your module supporting configuration of underflow events here
...
void displayInit()
{
	...
	gpioEnableDisplay();
	//si7021_enable(); // we need SENSOR_ENABLE=1 which is tied to DISP_ENABLE for the LCD, on all the time now
#else
#warning "gpioEnableDisplay is not implemented, please implement in order to use the display"
	...
	//timerEnable1HzSchedulerEvent(Scheduler_DisplayUpdate);
	  ...
	  // We will get a gecko_evt_hardware_soft_timer_id event back to gecko_wait_event()
	  struct gecko_msg_hardware_set_soft_timer_rsp_t     *timer_response;

	  timer_response = gecko_cmd_hardware_set_soft_timer(
			              32768,           // 1 second
			              0,               // timer handle, up to 256 handles are available
			              0);              // =0 timer runs repeating, =1, timer runs once
...
#warning "gpioSetDisplayExtcomin is not implemented.  Please implement for display support"
	...
	LOG_DEBUG("Extcomin state is %s",display->last_extcomin_state_high);
	return true;
